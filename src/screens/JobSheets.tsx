...
import { ArrowLeft, ArrowRight, BookOpen, CheckCircle, Star, Truck, Package, Bus, Info, QrCode } from 'lucide-react';
...
export const JobSheets = () => {
  ...
  const handleShareQr = (job: JobProfile) => {
    const url = `https://www.aftral.com/metiers/${job.id}`;
    setQrItem({ title: job.title, url });
    trackQrScan();
  };
      ...
      <div className="h-full flex flex-col animate-fadeIn">
        <div className="flex items-center justify-between mb-6">
          <button
            ...
            className="flex items-center gap-2 text-gray-600 font-bold hover:text-aftral-red transition-colors"
          >
            <ArrowLeft size={24} />
            Retour à la liste
          </button>
          <button
            onClick={() => handleShareQr(selectedJob)}
            className="flex items-center gap-2 bg-gray-100 hover:bg-blue-50 text-gray-600 hover:text-blue-600 font-bold px-5 py-3 rounded-xl transition-colors border border-gray-200 hover:border-blue-300"
          >
            <QrCode size={20} />
            Partager
          </button>
            ...
            <div className="grid grid-cols-1 gap-8">
              <section>
                <h3 className="text-xl font-bold text-aftral-dark mb-4 flex items-center gap-3">
                  <div className="bg-aftral-red/10 p-2 rounded-lg text-aftral-red"><CheckCircle size={24} /></div>
              ...
              <section>
                <h3 className="text-xl font-bold text-aftral-dark mb-4 flex items-center gap-3">
                  <div className="bg-aftral-red/10 p-2 rounded-lg text-aftral-red"><Star size={24} /></div>
                  Compétences clés
                </h3>
              ...
              <section>
                <h3 className="text-xl font-bold text-aftral-dark mb-4 flex items-center gap-3">
                  <div className="bg-aftral-red/10 p-2 rounded-lg text-aftral-red"><BookOpen size={24} /></div>
                  Formations AFTRAL
                </h3>
                <div className="space-y-3">
                  {selectedJob.trainingPaths.map((path, idx) => (
                    <div key={idx} className="flex items-center gap-3 text-gray-700 p-3 bg-blue-50 border border-blue-100 rounded-lg">
                      <div className="bg-white p-1 rounded-full"><ArrowRight size={16} className="text-blue-600" /></div>
      ...
      <div className="grid grid-cols-1 gap-4 overflow-y-auto pb-20">
        ...
        {newsTeaser ? (
          ...
          <div className="bg-gray-800 text-white rounded-2xl p-6 mt-4 flex items-center justify-between">
            ...
            <div className="bg-white/10 p-3 rounded-full"><BookOpen size={24} /></div>
          </div>
        )}
      </div>
