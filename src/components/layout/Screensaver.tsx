import { useEffect, useState, useMemo, FC, ElementType } from 'react';
...
export const Screensaver: FC<ScreensaverProps> = ({ onWake }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 bg-black text-white cursor-pointer overflow-hidden" onClick={onWake}>
            ...
            {/* Persistent CTA with Pulse Glow */}
            <div className="absolute bottom-12 left-0 right-0 z-50 flex justify-center">
                <div className="relative">
                    ...
                    <div className="absolute -inset-3 rounded-full bg-white/10 animate-pulse-ring" style={{ animationDelay: '0.5s' }} />
                    <div className="animate-bounce flex items-center gap-4 bg-white text-aftral-dark px-10 py-5 rounded-full shadow-[0_0_30px_rgba(255,255,255,0.3)] border-4 border-white/50 backdrop-blur-sm animate-pulse-glow relative z-10">
                        <div className="bg-aftral-red p-2 rounded-full text-white">
                            <MousePointerClick size={32} />
                        </div>
                        <span className="text-2xl font-bold uppercase tracking-wider">{t('screensaver.touch')}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="absolute top-0 left-0 right-0 h-2 bg-gray-800 z-50">
                <div
                    key={activeSlide}
                    className="h-full bg-aftral-red animate-[progress_10s_linear_forwards]"
                />
            </div>
        </div>
    );
